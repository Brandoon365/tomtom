 ...
 * 
 * 2013-06-25 cunningham : Added fix for IE8 when displaying long menus
 */
...
$dk=_build(dropdownTemplate,data);$dk.find('.dk_toggle').css({'width':width+'px'});$select.before($dk);$dk=$('#dk_container_'+id).fadeIn(settings.startSpeed);theme=settings.theme?settings.theme:'default';$dk.addClass('dk_theme_'+theme);data.theme=theme;data.$dk=$dk;$select.data('dropkick',data);$dk.data('dropkick',data);lists[lists.length]=$select;if(!$.browser.msie){$dk.bind('focus.dropkick',function(e){$dk.addClass('dk_focus');}).bind('blur.dropkick',function(e){$dk.removeClass('dk_open dk_focus');});}else{$dk.bind('focus.dropkick',function(e){$dk.addClass('dk_focus');});$('body').click(function(event){if(!$(event.target).parents('.dk_container').length){_closeDropdown($dk);$dk.removeClass('dk_focus');}});}
